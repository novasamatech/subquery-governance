...
import { addDelegateIdToVotings, addDelegatorActiveVotings, removeDelegateIdFromVotings, removeDelegatorActiveVotings } from "./voting";
...
export async function handleDelegate(call: CallBase<AnyTuple>, callOriginAddress: string, block: SubstrateBlock): Promise<void> {
  ...
  const [trackId, to, conviction, amount] = call.args as [Codec, Codec, Codec, Codec];
  ...
  const isFirstDelegationToThisDelegate = otherDelegatorDelegations.find(delegation => delegation.delegateId == delegateId) == undefined;
...
export async function handleUndelegate(call: CallBase<AnyTuple>, callOriginAddress: string): Promise<void> {
  const delegatorAddress = callOriginAddress;
  const [trackId] = call.args as [Codec];
  ...
  const wasLastDelegationToThisDelegate = otherDelegatorDelegations.find(delegation => delegation.delegateId == delegate.accountId) == undefined;
