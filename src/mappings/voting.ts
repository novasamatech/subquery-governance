...
import { Delegation, Referendum, CastingVoting, DelegatorVoting, ConvictionVote, StandardVote, SplitVote, SplitAbstainVote } from "../types";
...
export async function handleVote(call: CallBase<AnyTuple>, callOrigin: string, block: SubstrateBlock): Promise<void> {
  const [referendumIndex, accountVote] = call.args as [Codec, Codec];
...
export async function handleRemoveVote(call: CallBase<AnyTuple>, callOrigin: string): Promise<void> {
  const [trackId, referendumIndex] = call.args as [Codec, Codec];
...
async function updateVoting(voting: CastingVoting, accountVote: AccountVote, block: SubstrateBlock): Promise<void> {
  ...
  if (!isStandardBefore && isStandardAfter) {
    ...
    if (referendum && voting.delegateId) {
...
export async function removeDelegateIdFromVotings(delegateAddress: string): Promise<void> {
  ...
  await updateDelegateIdForCastingVotes(delegateAddress, vote => {
    vote.delegateId = undefined;
  });
}

async function updateDelegateIdForCastingVotes(voter: string, update: (vote: CastingVoting) => void): Promise<void> {
...
async function addDelegatorVotings(parentVotingId: string, delegateId: string, trackId: number): Promise<void> {
  ...
  const trackDelegations = allDelegations.filter(delegation => {
    return delegation.trackId == trackId;
  });
...
async function getDelegationByDelegateId(delegateId: string) {
  return await Delegation.getByDelegateId(delegateId, unboundedQueryOptions);
}

async function getCastingVotingByVoter(voter: string) {
  return await CastingVoting.getByVoter(voter, unboundedQueryOptions);
}

async function getDelegatorVotingByParentId(parentId: string) {
  return await DelegatorVoting.getByParentId(parentId, unboundedQueryOptions);
}

function extractStandardVote(accountVote: AccountVote): StandardVote | undefined {
...
function extractSplitVote(accountVote: AccountVote): SplitVote | undefined {
...
function extractSplitAbstainVote(accountVote: AccountVote): SplitAbstainVote | undefined {
